...
for (k in 1:nlam){  
 ...
 pdf(file="sim1_n2000_ncat7_ncov2dic.pdf",width=12,height=10)
 ...
 for (k in 1:nlam){
   ...
   main.text <- substitute(expression(lambda==la, alpha==al,df=edf, n=en, m==em),
                           list(la=lam[k], al=a, edf=df[k], en=1000,em=nrow(ris_ok[[k]]), 
                                expression=""))
...
save.image(file="sim1_n2000_ncat7_ncov2dic.RData")
