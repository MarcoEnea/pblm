...
data(bms)
ta <- xtabs(freq ~ fathers + sons, data=bms)+10
taz <- ta
...
for (k in 1:nlam){  
 ...
 #save.image(file="sim2_n5000_ncat7_ncov2dic.RData")
 
  ######################### Histograms ###############################
 load(file="sim2_n5000_ncat7_ncov2dic.RData")
 par(mfrow=c(3,3))
 pdf(file="sim2_n5000_ncat7_ncov2dic.pdf",width=12,height=10)
 ...
 for (k in 1:nlam){
   ...
   main.text <- substitute(expression(lambda==la, alpha==al,n=en, m==em),
                           list(la=lam[k], al=a, en=5000,em=nrow(ris_ok[[k]]), 
                                expression=""))
