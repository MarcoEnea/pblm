...
taz <- ta
prob <- as.vector(t(taz/sum(taz)+0.0001))
prob <- prob/sum(prob)
...
save.image(file="sim2_n2000_ncat7_ncov2dic.RData")
############################# Histograms ###############################
par(mfrow=c(3,3))
pdf(file="sim2_n2000_ncat7_ncov2dic.pdf",width=12,height=10)
...
for (k in 1:nlam){
  ...
  main.text <- substitute(expression(lambda==la, alpha==al,n=en, m==em),
                          list(la=lam[k], al=a, en=2000,em=nrow(ris_ok[[k]]), 
                               expression=""))
