...
#                b_30       b_31       b_32
         veroeta[10:25], rep(0.2,16), rep(0,16))
########################################################################
...
for (k in 1:nlam){  
  ...
  Iee[[k]] <- mod.false[[k]]$IM[30:45,30:45]
  Ibb[[k]] <- mod.false[[k]]$IM[1:29,1:29]
  Ibe[[k]] <- mod.false[[k]]$IM[1:29,30:45]
  Ieb[[k]] <- t(Ibe[[k]])
  Ieeb[[k]] <- (Iee[[k]] - Ieb[[k]]%*%solve(Ibb[[k]])%*%Ibe[[k]])
  autov[[k]] <- eigen(Ieeb[[k]]%*%solve(Ieeb[[k]]+mod.false[[k]]$P[30:45,30:45]))$values  
...
pdf(file="sim_n1000_ncat5_ncov2dic.pdf",width=12,height=10)
...
for (k in 1:nlam){
  ...
  main.text <- substitute(expression(lambda==la, alpha==al, df==def, n=en, m==em),
                          list(la=lam[k], al=a, def=round(df[k],digits=2),en=1000,
                               em=nrow(ris_ok[[k]]), 
                               expression=""))
  ...
  lines(seq(0,max(ris_ok[[k]]$LRT),l=500),dchisq(seq(0,max(ris_ok[[k]]$LRT),l=500),df[k]),
        col=2,lty=2)   
...
#save.image(file="sim_n1000_ncat5_ncov2dic.RData")
